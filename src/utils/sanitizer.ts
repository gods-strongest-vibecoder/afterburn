...
export function redactSensitiveData(text: string): string {
  ...
  // API key prefixes (OpenAI, GitHub, AWS, Anthropic, Stripe, Gemini, etc.)
  ...
  // Gemini API keys (format: AIza + 35 chars of alphanumeric/underscore/hyphen)
  redacted = redacted.replace(/\b(AIza[A-Za-z0-9_-]{35})/g, '[REDACTED_GEMINI_KEY]');
 ...
 */
export function sanitizeForMarkdownInline(text: string): string {
  if (!text) return '';
  return text
    .replace(/\|/g, '\\|')
    .replace(/`/g, '\\`')
...
/**
 * Escape HTML entities to prevent XSS in HTML reports.
