...
import type { FormInfo, FormField, InteractiveElement, LinkInfo } from '../types/discovery.js';
 ...
 */
export async function discoverElements(page: Page, pageUrl: string): Promise<DiscoveredElements> {
    ...
    for (let i = 0; i < roleElements.length; i++) {
      ...
      try {
        ...
        elements.otherInteractive.push({
          type: role as any, // TypeScript will coerce to InteractiveElement['type']
 ...
 */
export async function discoverHiddenElements(
  page: Page,
  pageUrl: string
  ...
  const newElements: DiscoveredElements = {
    forms: [],
    buttons: [],
    links: [],
    menus: [],
