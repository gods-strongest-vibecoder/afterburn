...
import type { SitemapNode, WorkflowPlan, PageData, FormInfo, InteractiveElement } from '../types/discovery.js';
...
Return JSON with a "workflows" array where each workflow has: workflowName, description, steps (action, selector, value, expectedResult, confidence), priority, estimatedDuration.`;
      ...
      const workflows = response.workflows
        .map(workflow => {
          ...
          // Determine source
          const isUserHint = userHints?.some(hint =>
            workflow.workflowName.toLowerCase().includes(hint.toLowerCase()) ||
            workflow.description.toLowerCase().includes(hint.toLowerCase())
          ) ?? false;
          ...
          return {
            ...
            description,
            steps: filteredSteps,
            source: isUserHint ? 'user-hint' as const : 'auto-discovered' as const,
          };
        })
        ...
        .sort((a, b) => {
          ...
          if (aPriority === bPriority) {
            if (a.source === 'user-hint' && b.source !== 'user-hint') return -1;
            if (a.source !== 'user-hint' && b.source === 'user-hint') return 1;
          }
   ...
   */
  private summarizeSitemap(node: SitemapNode, options?: { maxChars?: number }): string {
   ...
   */
  private collectPages(node: SitemapNode): SitemapNode[] {
   ...
   */
  private getPageImportance(pageData: PageData): number {
    let score = 0;
   ...
   */
  private formatPageSummary(node: SitemapNode, depth: number): string {
   ...
   */
  private formatForm(form: FormInfo): string {
