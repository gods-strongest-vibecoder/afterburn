 ...
 */
export async function auditMeta(page: Page): Promise<MetaAuditResult> {
  const url = page.url();
  ...
  try {
    ...
    if (!checks.hasOgTitle || !checks.hasOgDescription || !checks.hasOgImage) {
      const missing = [];
      if (!checks.hasOgTitle) missing.push('og:title');
      if (!checks.hasOgDescription) missing.push('og:description');
      if (!checks.hasOgImage) missing.push('og:image');
      issues.push({
