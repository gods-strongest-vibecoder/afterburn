...
export function validatePath(inputPath: string, label: string): string {
  ...
  if (trimmed.includes('..')) {
    throw new Error(
      `Path traversal detected in ${label}: "${trimmed}". Paths must not contain ".." sequences.`
    );
  }
...
export function validateSelector(selector: string): string {
  ...
  if (selector.length > MAX_SELECTOR_LENGTH) {
    throw new Error(
      `Selector too long (${selector.length} chars, max ${MAX_SELECTOR_LENGTH}). Possible injection attempt.`
    );
  }
  return selector;
}
