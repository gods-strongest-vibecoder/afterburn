...
export async function mapErrorToSource(
  ...
  // Race between search and timeout (with rejection guard for losing promise)
  return Promise.race([
    searchPromise().catch(err => { throw err; }), // Re-throw only if this wins
    timeoutPromise.catch(() => null) // Timeout promise never rejects, but guard anyway
  ]);
}
...
export async function mapMultipleErrors(
  ...
  // Race between batch search and timeout (with rejection guard for losing promise)
  await Promise.race([
    batchSearchPromise().catch(err => { throw err; }), // Re-throw only if this wins
    timeoutPromise.catch(() => {}) // Timeout promise never rejects, but guard anyway
  ]);
  return results;
}
