...
function isNonActionableFillField(field) {
...
function inferFieldFromSelector(selector) {
...
async function resolveFillTargetSelector(page, preferredSelector, fieldName) {
...
async function applyFillValue(page, selector, field, timeout, value) {
...
function getSkippedFillReason(field) {
    if (field.disabled)
        return 'Skipping disabled field';
    if (field.readOnly)
        return 'Skipping readonly field';
    if (field.hidden)
        return 'Skipping hidden field';
    return `Skipping unsupported fill field type: ${field.type}`;
}
async function tryFillSpecialField(page, selector, timeout, value) {
    const locator = page.locator(selector).first();
    const maybeLocator = locator;
    ...
    const field = await maybeLocator.evaluate((element) => {
        ...
        const type = tagName === 'select'
            ...
            : tagName === 'textarea'
                ? 'textarea'
                : (formElement.type || 'text').toLowerCase();
        ...
        return {
            ...
            label: (formElement.labels && formElement.labels.length > 0
            ...
            placeholder,
            disabled: formElement.hasAttribute('disabled') || formElement.disabled || false,
    ...
    const isSpecialFill = field.type === 'select' ||
