 ...
 */
function humanizeFormSelector(selector) {
    const idMatch = selector.match(/form#([\w-]+)/);
    if (idMatch)
        return idMatch[1].replace(/[-_]/g, ' ').replace(/\bform\b/gi, '').trim() + ' form';
    const classMatch = selector.match(/form\.([\w-]+)/);
    if (classMatch)
        return classMatch[1].replace(/[-_]/g, ' ').replace(/\bform\b/gi, '').trim() + ' form';
    return 'form';
}
/**
 ...
 */
function buildAccessibilityFix(violation) {
 ...
 */
function buildAccessibilityImpact(violation) {
    const id = violation.id.toLowerCase();
    if (id.includes('color-contrast') || id.includes('contrast')) {
 ...
 */
function buildJsErrorImpact(error) {
 ...
 */
function buildNetworkErrorImpact(error) {
    const msg = error.originalError.toLowerCase();
    if (msg.includes('404')) {
 ...
 */
function buildDeadButtonImpact(selector, buttonName) {
 ...
 */
function buildDeadButtonFix(selector, reason) {
    const hasTextMatch = selector.match(/:?has-text\("([^"]+)"\)/);
 ...
 */
function buildBrokenFormImpact(selector, formName) {
 ...
 */
function buildBrokenFormFix(form) {
 ...
 */
function findPageScreenshot(workflowResults) {
...
export function prioritizeIssues(diagnosedErrors, uiAudits, executionArtifact) {
    const issues = [];
    ...
    for (const error of diagnosedErrors) {
        if (['navigation', 'authentication', 'form'].includes(error.errorType)) {
            const priority = error.confidence === 'low' ? 'medium' : 'high';
            issues.push({
                priority,
    ...
    for (const error of diagnosedErrors) {
        if (error.errorType === 'network') {
            const priority = error.confidence === 'low' ? 'low' : 'medium';
            issues.push({
                priority,
                category: 'Console Error',
                summary: error.summary,
    ...
    for (const pageAudit of executionArtifact.pageAudits) {
        if (pageAudit.metaIssues) {
            for (const metaIssue of pageAudit.metaIssues) {
                const priority = metaIssue.severity === 'high' ? 'medium' : 'low';
                issues.push({
                    priority,
                    category: 'SEO / Meta',
 ...
 */
function extractResourceUrl(issue) {
