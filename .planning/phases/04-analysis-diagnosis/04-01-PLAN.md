...
must_haves:
  truths:
    ...
    - "Error diagnosis works even without AI API key"
  artifacts:
...
4. `DiagnosedError` type - combines ErrorDiagnosis with:
   - `originalError`: string ("Original error message from execution step, used for source mapping")
   - optional `sourceLocation` (SourceLocation)
   - optional `screenshotRef` (string path)
...
Add 1-2 line header comment to diagnosis-schema.ts.
  ...
  <done>All Zod schemas defined with proper TypeScript type inference. DiagnosedError includes originalError field for downstream source mapping. GeminiClient extended with generateStructuredWithImage method that accepts image file path. Both files compile without errors.</done>
</task>
...
1. `analyzeErrors(artifact: ExecutionArtifact, options?: { apiKey?: string }): Promise<DiagnosedError[]>` - main export function that:
   ...
   - IMPORTANT: Each DiagnosedError must populate `originalError` with the raw error message from the step/evidence (e.g., `stepResult.error` string). This field is used by Plan 04-03 for source code mapping.
...
2. LLM diagnosis flow (`diagnoseWithLLM`):
   ...
   - Attach `originalError: stepResult.error` to the resulting DiagnosedError
...
3. Fallback diagnosis (`fallbackDiagnosis`):
   ...
   - Populate `originalError` with the raw error message for each fallback diagnosis too
   - Set aiPowered: false
...
Add 1-2 line header comment to error-analyzer.ts.
  ...
  <done>Error analyzer processes ExecutionArtifact into DiagnosedError array with LLM-powered root cause analysis. Each DiagnosedError includes originalError field with the raw error message. Fallback mode works without API key using pattern matching. All exports available from src/analysis/index.ts.</done>
</task>
...
3. DiagnosedError type includes `originalError: string` field
4. src/ai/gemini-client.ts has generateStructuredWithImage method accepting imagePath
5. src/analysis/error-analyzer.ts exports analyzeErrors function
6. src/analysis/index.ts barrel exports all public APIs
</verification>

<success_criteria>
- All Phase 4 Zod schemas defined and type-safe
- DiagnosedError includes originalError field for source mapping in Plan 04-03
