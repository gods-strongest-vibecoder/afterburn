...
<task type="auto">
  ...
  <action>
    ...
    1. Imports (all of these MUST be present at the top of the file):
       ...
       Note: The `PrioritizedIssue` type import is critical -- it is used in `generateIssueTable()` and throughout the report.
    ...
    **AI-parseability validation (build these into the implementation above):**
    - YAML frontmatter MUST be properly delimited with `---` markers (opening and closing)
    - Heading hierarchy MUST be: H1 for title, H2 for sections, H3 for subsections (no skipped levels)
    - All tables MUST have header rows with `|` delimiters and a separator row (`|---|---|`)
    - Code blocks MUST use triple backticks with language hints (```typescript, ```bash)
    - Use explicit text labels for metadata: `**Priority:** HIGH` (not emoji-only markers)
    - Source code references MUST use backtick-quoted file paths: `src/file.ts:42`

    **Edge case handling (build into each helper function):**
    - Empty `diagnosedErrors` array: `generateTechnicalDetails` returns "No errors diagnosed."
    - Empty `uiAudits` array: `generateUIAuditSection` returns "No UI audits performed."
    - No source analysis: `generateSourceReferences` returns "No source code references available. Run with `--source ./path` for source-level diagnosis."
    - All workflows passing: `generateReproductionSteps` returns "All workflows passed successfully."
    - Zero workflows: graceful handling in frontmatter and summary (no division by zero)
  </action>
  <verify>
    Run `npx tsc --noEmit` to verify types compile. Verify the file exports `generateMarkdownReport` and `writeMarkdownReport`. Verify edge case handling: each helper function returns a fallback message for empty input arrays.
  </verify>
  <done>
    markdown-generator.ts exports generateMarkdownReport (returns Markdown string with YAML frontmatter) and writeMarkdownReport (saves to file). Report includes all required sections: frontmatter metadata, prioritized issue table, technical error details, reproduction steps, UI audit results, accessibility summary, and source code references. AI-parseability ensured: proper YAML frontmatter, consistent heading hierarchy (H1/H2/H3), labeled tables, language-hinted code blocks, explicit priority labels. All edge cases handled gracefully with fallback messages.
  </done>
</task>
