...
4. Call `runAfterburn()` with parsed options
...
Vibe coder tone: "Afterburn v0.1.0" not "Afterburn Automated Web Testing Platform v0.1.0". Keep it clean.
  ...
  <verify>Run `npx tsc --noEmit` to verify no type errors. Run `node dist/index.js --help` after build to verify Commander help text displays correctly with all flags listed.</verify>
  <done>Commander.js CLI works with all flags. `--help` shows usage. src/index.ts is a 3-line entry point. package.json has "files" field. No type errors.</done>
</task>
