...
<task type="auto">
  <name>Task 2: Create Action entry point with PR comments, artifact upload, and fail-on threshold</name>
...
4. Verify fail-on threshold logic is correct by reading the compiled code and confirming:
   - `fail-on: 'high'` only calls setFailed when highPriorityCount > 0
   - `fail-on: 'medium'` calls setFailed when highPriorityCount + mediumPriorityCount > 0
   - `fail-on: 'low'` calls setFailed when totalIssues > 0
   - `fail-on: 'never'` never calls setFailed
5. Grep action/index.ts for the fail-on logic: confirm all four cases (high, medium, low, never) are handled
  </verify>
  <done>action/index.ts compiles without errors. action.yml defines correct inputs/outputs/runs. PR comment builder creates scannable summary. Artifact upload code present. Failure threshold logic correctly implements all four levels: high (fails on high only), medium (fails on high+medium), low (fails on any), never (never fails).</done>
</task>
...
6. fail-on logic verified: 'high' only fails on high-priority, 'medium' fails on high+medium, 'low' fails on any issue, 'never' never fails
</verification>
...
- fail-on threshold logic works correctly for all four levels: high, medium, low, never
