...
must_haves:
  truths:
    ...
    - "MCP sends progress notifications during long-running scans (when SDK supports it, otherwise gracefully omits them)"
    - "When scan_website encounters an error, MCP returns error message to client and server stays responsive for next request"
  artifacts:
...
- Handler:
  ...
  2. Use `onProgress` callback to send MCP progress notifications. Check the SDK for progress notification support:
     - If the SDK provides a progress notification method (e.g., `server.notification()` or `meta.progressToken`), implement progress by sending stage updates during scan
     - If the SDK does NOT have straightforward progress support, log a warning and proceed without progress notifications — a working tool without streaming is better than a broken tool with streaming
     - Either way, this is NOT a failure. The must_have truth accounts for SDK variance.
...
Error handling: Wrap runAfterburn call in try/catch. On error, return `isError: true` with error message as text content. Do NOT let the server crash — it must remain responsive for subsequent requests.
...
5. Verify error handling: after a failed scan_website call, sending another initialize should still get a response (server stays responsive)
  </verify>
  <done>MCP server compiles, starts via stdio, responds to initialization. scan_website tool is registered with correct input schema. Error responses return isError: true without crashing the server. Package.json has afterburn-mcp bin entry.</done>
</task>
...
7. Error in runAfterburn returns MCP error response (isError: true), server process does not exit
</verification>
...
- Progress notifications sent when SDK supports them (gracefully omitted otherwise)
- Errors are caught and returned as MCP error responses — server stays responsive for next request
