...
<task type="auto">
  <name>Task 2: Build recursive web crawler with URL queue, pageProcessor callback, and additionalUrls support</name>
...
**IMPORTANT — pageProcessor callback:**
The `pageProcessor` callback is called for each crawled page BEFORE the page is closed. This allows the discovery pipeline (Plan 05) to inject per-page processing (element discovery, screenshot capture, link validation) without the crawler needing to know about those modules.
...
4. Create minimal PageData (url, title, links, crawledAt timestamp)
5. If pageProcessor exists: `const extra = await pageProcessor(page, url)` — merge returned Partial<PageData> into the PageData via `Object.assign(pageData, extra)`
6. Close page: `await page.close()`
7. Add discovered links to queue (same-hostname, not visited)

This separates crawling (URL management, dedup, concurrency) from per-page processing (element discovery, screenshots, link validation).

**IMPORTANT — additionalUrls support:**
The `additionalUrls` option seeds extra URLs into the crawl queue at startup. This is how SPA routes discovered by `interceptRouteChanges()` (Plan 02) get crawled. The pipeline discovers SPA routes on the first page, then passes them as `additionalUrls` when constructing SiteCrawler.

In the `crawl()` method:
1. Add seed URL to queue
2. If `additionalUrls` provided: add each to queue (after normalizing and filtering to same hostname)
3. Proceed with normal crawl loop

**crawl() method implementation:**

1. Parse seed URL to extract hostname
2. Add seed URL to queue
3. Add additionalUrls to queue (normalize, filter same-hostname, skip duplicates)
4. Process queue in batches of maxConcurrency:
   ...
   - Create a PageData object (url, title, links extracted, crawledAt timestamp). If pageProcessor provided, call it and merge result before closing page. Leave forms/buttons/menus as empty arrays by default — the pageProcessor callback populates them.
   - Close the page after processing
   - Mark URL as visited
5. After 50 pages, log a warning: "Discovered 50+ pages. Crawl continuing..."
6. If maxPages > 0, stop after that many pages
7. Return CrawlResult with pages array, empty brokenLinks (link validator fills later), counts, and duration
...
(after `npm run build`)
  ...
  <done>SiteCrawler class crawls all same-hostname pages using BrowserManager, deduplicates URLs, respects concurrency limits, supports pageProcessor callback for per-page processing, accepts additionalUrls for SPA route seeding, and returns CrawlResult with PageData array.</done>
</task>
...
- SiteCrawler accepts pageProcessor callback in CrawlerOptions
- SiteCrawler accepts additionalUrls in CrawlerOptions for SPA route seeding
...
- pageProcessor callback called per page before page.close()
- additionalUrls seeded into queue at crawl start
